...
detection_file = os.path.join(mypath,'evaluation','Car_detection_results.txt')
...
def parse_labels(dets_df, gt_dir, val_list):
...
if __name__ == '__main__':
    ...
    for frame in frames:
        ...
        for index, row in frame_df.iterrows():
            ...
            bbdet = row['bbdet']
            print(bbdet)
            out_text += '{} {} {} {} '.format(bbdet[0],bbdet[1],bbdet[2],bbdet[3])
            out_text += '0 0 0 0 0 0 0 '
            out_text += '{}'.format(row['confidence'])
            out_text += '\n'
        fp.write(out_text)
        fp.close()

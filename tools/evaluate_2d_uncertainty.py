...
detection_file = os.path.join(mypath,'results','vehicle.car_detection_results.txt')
...
def plot_histo_bbox_uc(dets,scene,min_val,max_val):
    ...
    for column in bboxes:
        if('e_bbox_var' in column):
            ...
            #mean    = np.mean(hist_data)
            #hist_data = (hist_data-min_val)/(max_val-min_val)
            plt.hist(hist_data,bins=20,range=[min_val,max_val],alpha=0.5,label=labelname,density=True,stacked=True)
    #bboxes = bboxes.to_dict(orient="list")
    return None 
...
if __name__ == '__main__':
    ...
    #plot_histo_cls_uc(sun_dets,'sun',minm,maxm)
    #plot_histo_bbox_uc(night_dets,'night',minm,maxm)
    plot_histo_bbox_uc(day_dets,'sun',minm,maxm)
    plot_histo_bbox_uc(rain_dets,'rain',minm,maxm)
    plt.legend()
    plt.show()
    #print(day_dets)
